  ...
  <div class="bg-white max-w-lg mx-auto p-8 md:p-12 mt-14 rounded-lg shadow-2xl">
    ...
    <form class="flex flex-col" @submit.prevent="UpdateProfile()">
      ...
      <div class="mb-6 pt-3 rounded bg-gray-200">
          ...
          <input type="text" id="username" v-model.trim="$auth.user.username"
      ...
      <div class="mb-6 pt-3 rounded bg-gray-200">
          ...
          <input type="email" id="email" v-model.trim="$auth.user.email"
          class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-purple-600 transition duration-500 px-3 pb-3">
      </div><!-- email -->
      <!-- password -->
      <div class="mb-6 pt-3 rounded bg-gray-200">
    ...
    <form class="flex flex-col" @submit.prevent="UpdatePassword()">
      ...
      <div class="mb-6 pt-3 rounded bg-gray-200">
          <label class="block text-gray-700 text-sm font-bold mb-2 ml-3" for="password">Older Password*</label>
          <input autocomplete="on" type="password" id="password" v-model.trim="formPassword.currentPassword"
          class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-purple-600 transition duration-500 px-3 pb-3">
      </div><!-- password -->
      <!-- password -->
      <div class="mb-6 pt-3 rounded bg-gray-200">
  ...
  methods : {
    ...
    UpdateProfile() {
      ...
      this.$axios
        ...
        .then(() => {
          this.formProfile.success = true
        })
        .catch((error) => {
          this.formProfile.error = error
        })
    },
    ...
    UpdatePassword() {
      ...
      // no fields empty
      if (!this.formPassword.currentPassword || !this.formPassword.newPassword || !this.formPassword.confirmNewPassword) return (this.formPassword.error = 'All fields are necessary')
      ...
      this.$axios
        .post(`/api/ROUTE`, {
          token : this.$auth.user._id,
          password: this.formPassword.currentPassword,
          newPassword: this.formPassword.newPassword,
        })
        .then(() => {
          this.formPassword.success = true
        })
        .catch((error) => {
          this.formPassword.error = error
        })
    }
  }
}
